...
def init(Name):
    ...
    if Name == 'cantilever':
        ...
        #if Load is None:
            #raise ValueError(f"Error: Load is None for structure '{Name}'. Define Load values.")

        #return Lag, Nx, Ny, lx, ly, Load, Name, ds, bcd, mesh, phi_mat, Vvec
        #return mesh, Vvec, bcd, Load
    ...
    if Name == 'bridge':
        ...
        # Initialize level set function
        #bcd = [DirichletBC(Vvec, (0.0,0.0), dirBd2, method="pointwise")] #doesn't call the 1st boundary condition

        phi_mat = -np.cos((6.0*pi*(XX-1.0))) * np.cos(2*pi*YY) - 0.4 \
         + np.maximum(100.0*(YY-ly+0.1),.0)
