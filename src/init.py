...
def init(Name):
    ...
    if Name == 'half_wheel' or Name == 'bridge':
        ...
        free_height = 0.3
        clamped_height = ly - free_height 
        
        #left fixed vertical segment (0 < y < 1.7)
        class DirBd(SubDomain):
            def inside(self, x, on_boundary):
                return (on_boundary
                        and abs(x[0]) < tol
                        and x[1] <= clamped_height + tol) 
        
        #right fixed vertical segment (0 < y < 1.7)
    ...
    if Name == 'bridge':
        ...
        # Initialize level set function
        phi_mat = -np.cos((2.0*pi*(XX-1.0))) * np.cos(4*pi*YY) - 0.3 \
