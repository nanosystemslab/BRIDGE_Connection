...
#from dolfin import PETScLUSolve
# ----------------------------------------------------------------------
def _main():
    if len(sys.argv) < 3:
    ...
    OUT = Path(os.path.dirname(__file__)) / "OUT"
    ...
    while It < ItMax and stop == False:
        ...
        if It > 0 and J[It] > J[It-1] and ls < ls_max:
            ...
            if np.mod(It, 10) == 0 or It == 1 or It == ItMax or stop == True:
                ...
                # use your chosen label if provided, else fall back to job id
                run_label = os.environ.get("RUN_NAME", f"output_{job_number}")

                # when saving:
                fig.savefig(OUT / f"{run_label}_iteration_{It}.pdf", bbox_inches="tight")


                # Save the figure before showing it
                #pp.savefig(OUT/f'output_iteration_{It}.pdf', bbox_inches='tight')
                #fig.savefig(OUT / f"output_{job_number}_iteration_{It}.pdf", bbox_inches='tight')
